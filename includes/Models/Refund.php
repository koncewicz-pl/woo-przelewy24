...
class Refund
{
    ...
    const REFUND_LINE_ITEMS_PREFIX = '_p24_refund_line_items_';
    ...
    public function __construct(int $order_id, float $amount = 0, string $reason = '', ?array $line_items = null)
    {
    ...
    private function remove_pending_refund($request_id): void
    {
        ...
        $this->order->delete_meta_data(self::REFUND_LINE_ITEMS_PREFIX . $request_id);
        $this->order->update_meta_data(self::REFUND_PENDING, $this->pending_refunds);
    }

    private function get_stored_line_items(string $request_id): ?array
    {
        return (array)$this->order->get_meta(self::REFUND_LINE_ITEMS_PREFIX . $request_id);
    }
    ...
    public function verify(Refund_Notification $notification): void
    {
        ...
        if ($notification->status === 0) {
            ...
            $refund = wc_create_refund($attributes);
